...
export function Some<T>(val: NonNullable<T>): Option<T> {
    if (val === undefined || val === null) {
        throw new Error(
            `"{val}" passed into Some() constructor function. If this is a valid case - Maybe() should be used insted, otherwise you probably have a bug. Please verify non null assertions.`,
        );
    }
