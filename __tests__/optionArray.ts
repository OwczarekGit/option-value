...
test('Creates OptionArray from array.', () => {
  let animals = ['dog', 'cat', 'horse']
  let optArr = OptionArray.fromArray(animals);
  expect(optArr.length).toBe(3)
  expect(optArr.maybeAt(0).isSome()).toBe(true)
  expect(optArr.maybeAt(0).get()).toBe('dog')
  expect(optArr.maybeAt(1).isSome()).toBe(true)
  expect(optArr.maybeAt(1).get()).toBe('cat')
  expect(optArr.maybeAt(2).isSome()).toBe(true)
  expect(optArr.maybeAt(2).get()).toBe('horse')
})
